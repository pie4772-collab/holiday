...
AS_OF_DATE=2026-07-31  # Excel 연차 대장 스냅샷 기준일(7월 말 사용분)
```
...
- **회계 기준일**: 매년 **1월 1일**
- **첫해**: 입사 대응일마다 월 1개 (최대 11), 일사일에 정산
- **비례 연차**: 1년 도달 ~ 첫 1/1 전 · `15 × (남은일수/365)` 후 반올림(0.1~0.4→0.5, 0.6~0.9→1)
- **정규 연차**: 매 1/1 발생
- **근속 가산**: 정규 연차 **최초 발생일**(비례 연차 이후 첫 1/1)부터 계산
- **잔여 표시**: 사용 > 발생이면 **음수** 표시 (연차 관리·월말 보고서)
- **초과 이월**: 잔여가 음수면 다음 주기 **발생분에서 초과분만큼 차감**
- **IFRS 연차부채**: 부채·수당 산정 시 잔여는 **최소 0** (`getPayableLeaveDays`)
- **임원 제외**: 이사·상무·전무·대표이사·임원은 연차 발생·신청·정산·부채 대상에서 제외
- **공장장 결재**: 공장장 본인 신청 시 추가 결재 없이 즉시 승인
- **승인 메일**: 결재권자 승인 시 신청자·승인자·해당 사업장 관리자에게 발송
- **승인·반려 이력**: `leave_approval_logs`에 신청/단계승인/최종승인/본인승인/반려를 스냅샷 보관 (관리자 메뉴 · CSV)

### 연차수당·통상 일급 (`server/services/leaveService.js`)

- 일급 = `floor(월통상임금 ÷ 209 × 8)` — **원 미만 버림(절사)**
- 수당/부채 금액 = `round(일급 × max(0, 잔여일수))` — 원 단위 반올림
- **IFRS 연차부채** 통상임금: `employees.ordinary_wage` (지난달 월말 확정분 불러오기 가능)
- **연차 정산** 통상임금: `leave_settlement_ordinary_wages` — 부채와 분리, 정산 시 관리자 직접 입력 (자동 연동 없음)
- 적용 화면: IFRS 연차부채, 연차 정산

### Excel 스냅샷

- 기준일 기본값: **2026-07-31** (7월 말 사용분까지)
- 스냅샷은 **현재 연차 주기 안**이고 조회일 ≥ 기준일일 때만 사용
- 주기 전환(비례·다음 해 정규) 후에는 엔진 계산
- 첫해: 스냅샷 이후 입사 대응일 월차·과소계상을 발생·잔여에 가산
- 스냅샷 사용 중에는 전기 이월 중복 차감 없음 (엑셀 잔여에 반영된 것으로 봄)
...
- **통상 일급 절사** — `(월통상임금 ÷ 209 × 8)` 계산 후 **소수점 이하 버림** (`Math.floor`). 이전의 소수 2자리 반올림 폐지
- **연차수당/IFRS** — 일급×잔여 금액은 기존처럼 원 단위 반올림 유지, 부채 잔여 일수는 최소 0

### 2026-09-15

- **스냅샷 잔액 보정** — 주기·기준일 게이트, 첫해 월차 사후 가산, 비례 구간 스냅샷 오적용 방지, `AS_OF_DATE` 기본 2026-07-31
- **비례 연차 반올림** — 0.1~0.4→0.5, 0.5 유지, 0.6~0.9→1
- **잔여 음수 표시** — 연차 관리·월말 보고서에서 초과 사용 시 음수 잔여 표시
- **초과분 차년도 차감** — 전기 초과 사용을 다음 주기 발생분에서 차감 (집계 기준일 오염 버그 수정 포함)
- **당해 누적 사용** — 직원 상세 기본 정보에 표시
- **연차 관리 필터 유지** — 사업장·부서·상태·검색·정렬을 sessionStorage에 저장, 상세/수정 복귀 시 유지
- **브랜드** — KBI 심볼 + `KBI 동양철관` 텍스트, 로그인 화면 좌우 배치
- **로고 배포 수정** — 마크를 Vite 번들(`src/assets`)로 포함해 Cafe24에서 이미지 404/HTML 폴백 방지
...
- **Excel 연차 대장 임포트** — 74명, `AS_OF_DATE=2026-07-31` 기준 스냅샷
